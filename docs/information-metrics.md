...
lower number or a nonconverged solve. The verifier recomputes the Lagrangian or
supported-endpoint lower bound from the stored action marginal and problem, and
checks the reported gap, so changing the reported lower value invalidates the
certificate.
